...
COVID_Raw['Tested_Positive_Ratio'] = (
    COVID_Raw['Positive_Raw'] / COVID_Raw['Tested_Raw']) * 100
...
with matplotlib.pyplot.xkcd():
    ...
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Cases –All Days Plot.png', dpi=900)

    CPDelta_df = COVID_Raw[['Positive_Raw']]
    deltaFig = CPDelta_df.last('14D').plot(
        title='Kosovo COVID-19 Cases –L14 Days')
    deltaFig.legend(loc='upper left')
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Cases –L14 Days Plot.png', dpi=900)
    ...
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Positive to Tested Ratio –All Days.png', dpi=900)
    ...
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Positive to Tested Ratio –L14.png', dpi=900)

    CPDelta_df = COVID_Raw[['Tested_Raw', 'Positive_Raw', 'Died_Raw']]
    deltaFig = CPDelta_df.last('14D').plot(
        title='Kosovo COVID-19 Cases Overall –Last 14 Days')
    deltaFig.legend(fontsize='x-small')
    deltaFig.minorticks_off()
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Cases Overall –Last 14 Days Plot.png', dpi=900)

    CPDelta_df = COVID_Raw[['Tested_Raw', 'Positive_Raw', 'Died_Raw']]
    deltaFig = CPDelta_df.last('7D').plot(
        title='Kosovo COVID-19 Cases Overall –Last 7 Days')
    deltaFig.legend(fontsize='x-small')
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Cases Overall –Last 7 Days Plot.png', dpi=900)

    CPDelta_df = COVID_Raw[['Tested_Raw', 'Positive_Raw']].last('14D')
    ticks = CPDelta_df.index.strftime('%m-%d').values
    deltaFig = CPDelta_df.plot(
        kind='bar', title='Kosovo COVID-19 Cases –Last 14 Days')
    deltaFig.set_xticklabels(ticks)
    deltaFig.minorticks_off()
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Cases –Last 14 Days Bar.png', dpi=900)

    CPDelta_df = COVID_Raw[['Tested_Raw', 'Positive_Raw',
                            'Tested_Positive_Ratio']].last('14D')
    ticks = CPDelta_df.index.strftime('%m-%d').values
    deltaFig = CPDelta_df.plot(
        kind='bar', title='Kosovo COVID-19 Cases –Last 14 Days', logy=True)
    deltaFig.set_xticklabels(ticks)
    ...
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Cases –Last 14 Days Bar.png', dpi=600)

    CPDelta_df = COVID_Raw[['Positive_Raw', 'Recovered_Raw', 'Died_Raw']]
    ticks = CPDelta_df.last('14D').index.strftime('%m-%d').values
    deltaFig = CPDelta_df.last('14D').plot.bar(
        stacked=True, title='Kosovo COVID-19 Cases Overall –Last 14 Days')
    deltaFig.legend(fontsize='xx-small')
    deltaFig.set_xticklabels(ticks)
    deltaFig.minorticks_off()
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Cases Overall –L14 Days Stacked Bar.png', dpi=900)

    CPDelta_df = COVID_Raw[['Positive_Raw', 'Recovered_Raw', 'Died_Raw']]
    ticks = CPDelta_df.last('14D').index.strftime('%m-%d').values
    deltaFig = CPDelta_df.last('14D').plot.area(
        stacked=True, title='Kosovo COVID-19 Cases Overall –Last 14 Days')
    deltaFig.legend(fontsize='xx-small')
    deltaFig.set_xticklabels(ticks)
    deltaFig.figure.tight_layout()
    deltaFig.figure.savefig(
        'Kosovo COVID-19 Cases Overall –L14 Days Area.png', dpi=900)
